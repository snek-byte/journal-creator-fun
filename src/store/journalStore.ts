...
export const useJournalStore = create((set, get) => ({
    ...
    updateIcon: (iconId, updates) => set((state) => {
        const updatedIcons = (state.currentEntry.icons || []).map((icon) => icon.id === iconId ? {
            ...icon,
            ...updates
        } : icon);
        return {
    ...
    removeIcon: (iconId) => set((state) => {
        const filteredIcons = (state.currentEntry.icons || []).filter((icon) => icon.id !== iconId);
        return {
            currentEntry: {
                ...state.currentEntry,
                icons: filteredIcons
    ...
    saveEntry: async () => {
        ...
        try {
            ...
            if (currentEntry.id) {
                const { error } = await supabase.from('journal_entries').update(entryData).eq('id', currentEntry.id);
                ...
                const lastEntryDate = userProgress.lastEntry ? new Date(userProgress.lastEntry).toDateString() : null;
                const yesterday = new Date();
                yesterday.setDate(yesterday.getDate() - 1);
                const isConsecutiveDay = lastEntryDate === yesterday.toDateString();
                const newStreak = isConsecutiveDay ? userProgress.streak + 1 : lastEntryDate === today ? userProgress.streak : 1;
    ...
    loadEntries: async () => {
        try {
            ...
            const { data, error } = await supabase.from('journal_entries').select('*').eq('user_id', user.id).order('created_at', {
                ascending: false
            });
            if (error) throw error;
            set({
    ...
    applyChallenge: () => {
        ...
        set((state) => ({
            currentEntry: {
                ...state.currentEntry,
                text: state.currentEntry.text + (state.currentEntry.text ? '\n\n' : '') + `Today's Challenge: ${dailyChallenge.prompt}\n\n`
            }
        }));
    }
}));
